...
CFLAGS_ATARI_MS = $(CFLAGS) -mshort -nostdlib -I$(LIBCMINI_DIR)/include
LDFLAGS_ATARI   = -lm -Wl,--traditional-format
LDFLAGS_ATARI_MS = -L$(LIBCMINI) -lcmini -lgcc -lcmini
LDFLAGS_LINUX   = -lm

SDL_CFLAGS = $(shell pkg-config --cflags sdl2)
SDL_LIBS   = $(shell pkg-config --libs sdl2)

all: SV2025.tos SV2025-ascii.tos SV2025-ascii-ms.tos sv2025-sdl sv2025-ascii

clean:
	rm -f *.o *.tos *.sym sv2025-sdl sv2025-ascii

# ── Atari object files (mintlib) ───────────────────────────────────────────────
...
# ── Atari object files (-mshort + libcmini) ────────────────────────────────────

SV2025_atari_ms.o: SV2025.c backend.h SV2025.h
	$(CC_ATARI) $(CFLAGS_ATARI_MS) -c $< -o $@

backend_ascii_atari_ms.o: backend_ascii.c backend.h
	$(CC_ATARI) $(CFLAGS_ATARI_MS) -c $< -o $@
...
SV2025.tos: SV2025_atari.o backend_gemtos.o segline.o clipline.o
	$(CC_ATARI) $^ -o $@ $(LDFLAGS_ATARI)
	gst2ascii $@ > SV2025.sym

SV2025-ascii.tos: SV2025_atari.o backend_ascii_atari.o
	$(CC_ATARI) $^ -o $@ $(LDFLAGS_ATARI)

SV2025-ascii-ms.tos: SV2025_atari_ms.o backend_ascii_atari_ms.o
	$(CC_ATARI) -mshort -nostdlib $(CRT0) $^ -o $@ $(LDFLAGS_ATARI_MS)
